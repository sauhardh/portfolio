        ...
        const SpanLogo = ({ LOGOS, offset }: { LOGOS: (() => React.JSX.Element)[], offset: number }) => {
            return (
                <div className="flex justify-around min-w-screen">
                    {LOGOS.map((Logo, i) => (
                        <span key={i + offset} className="py-0 h-10 md:w-14 md:h-14 lg:w-16 lg:h-16 max-[500px]:px-1 max-[400px]:px-1.5 flex items-center justify-center filter sepia contrast-50 brightness-100 grayscale">
                            <Logo />
                        </span>
                    ))}
                </div>
