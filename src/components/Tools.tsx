      ...
      return (
        <div className="flex justify-around min-w-screen">
          {LOGOS.map((Logo, i) => (
            <span
              key={i + offset}
              className="py-0 h-10 md:w-14 md:h-14 lg:w-16 lg:h-16 max-[500px]:px-1 max-[400px]:px-1.5 flex items-center justify-center filter sepia contrast-50 brightness-100 grayscale"
            >
              <Logo />
            </span>
          ))}
  ...
  return (
    <MagneticCursor text="Few among the tools I play with">
      <div className="flex flex-col justify-center items-center gap-4 sm:gap-6 my-12 sm:my-44 ">
        ...
        <DisplayLogos
          LOGOS={FIRST_ROW_LOGOS}
          top_div_class="mt-1"
          marquee_animation="animate-marquee"
        />
        <DisplayLogos
          LOGOS={SECOND_ROW_LOGOS}
          marquee_animation="animate-marquee-reverse"
        />
      </div>
    </MagneticCursor>
  );
